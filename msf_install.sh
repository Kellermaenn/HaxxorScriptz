...
function install_armitage_osx
{
	if [ -e /usr/bin/curl ]; then
		...
		curl -# -o /tmp/armitage.tgz http://www.fastandeasyhacking.com/download/armitage150813.tgz && print_good "Finished"
		if [ $? -eq 1 ] ; then
...
#######################################
function install_rvmruby
{
	print_status "Installing Ruby1.9.3"
...
function install_deps_deb
{
	...
	sudo apt-get -y install build-essential libreadline-dev  libssl-dev libpq5 libpq-dev libreadline5 libsqlite3-dev libpcap-dev postgresql-client-common openjdk-8-jre subversion git-core autoconf postgresql pgadmin3 curl zlib1g-dev libxml2-dev libxslt1-dev xtightvncviewer libyaml-dev #>> $LOGFILE
	
	install_rvmruby
	
	print_status "Installing base Ruby Gems"
	rvm all do gem install wirble sqlite3 bundler #>> $LOGFILE
}
#######################################
...
function install_nmap_linux
{
	...
	sudo apt-get install -y nmap #>> $LOGFILE
}
#######################################
